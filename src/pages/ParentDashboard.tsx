...
import {
  ...
  ChevronDown, ChevronUp, Settings2, Target, BookOpen, BarChart3, CalendarDays
} from 'lucide-react';
...
export default function ParentDashboard() {
  ...
  const SectionToggle = ({ label, icon: Icon, show, onToggle }: { label: string; icon: any; show: boolean; onToggle: () => void }) => (
    <button onClick={onToggle} className="flex items-center gap-2 text-gold font-bold text-sm mb-3 w-full">
      <Icon size={16} /> {label}
      {show ? <ChevronUp size={16} className="ms-auto" /> : <ChevronDown size={16} className="ms-auto" />}
    </button>
  );
  ...
  return (
    <div className="min-h-screen gradient-night p-4 pb-8">
      <div className="max-w-md mx-auto">
        ...
        {/* PIN Section */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-2xl p-5 border border-border mb-4">
        ...
        {/* Settings Section */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-2xl p-5 border border-border mb-4">
          <SectionToggle label="الإعدادات" icon={Settings2} show={showSettings} onToggle={() => setShowSettings(!showSettings)} />
          {showSettings && (
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2 items-center">
                ...
                <input value={rewardGoal} onChange={e => setRewardGoal(e.target.value)} type="number"
                  className="w-24 bg-muted border border-border rounded-xl px-3 py-2 text-foreground outline-none focus:ring-2 focus:ring-primary" dir="ltr" />
        ...
        {/* Gift Tiers */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-2xl p-5 border border-border mb-4">
          <SectionToggle label="هدايا متعددة" icon={Target} show={showGifts} onToggle={() => setShowGifts(!showGifts)} />
          {showGifts && (
            <div className="space-y-3">
              {giftTiers.map(tier => (
                <div key={tier.id} className="flex items-center gap-3 bg-muted rounded-xl p-3">
        ...
        {/* Custom Activities */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-2xl p-5 border border-border mb-4">
          <SectionToggle label="أنشطة إضافية" icon={BookOpen} show={showActivities} onToggle={() => setShowActivities(!showActivities)} />
          {showActivities && (
            <div className="space-y-3">
              <p className="text-muted-foreground text-xs">أضف أنشطة مثل: حفظ القرآن، مساعدة الأم، الصلاة في وقتها...</p>
              {activities.map(a => (
                <div key={a.id} className="flex items-center gap-3 bg-muted rounded-xl p-3">
        ...
        <div className="mb-4"><ReminderSettings /></div>
                ...
                <motion.div key={child.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.1 }}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex gap-3 text-sm flex-wrap">
                      ...
                      {moneyReward.enabled && money > 0 && <span className="text-muted-foreground">💰 <span className="text-secondary font-bold">{money} {moneyReward.currency}</span></span>}
                    </div>
                    {achieved && <p className="text-gold text-xs font-bold mt-1">🏆 حقق الهدف!</p>}
                    <div className="h-1.5 bg-muted rounded-full overflow-hidden mt-2">
                      <div className="h-full gradient-gold rounded-full transition-all" style={{ width: `${Math.min((progress.total / rewardData.goal) * 100, 100)}%` }} />
                    </div>
                  </div>
        ...
        {/* Edit specific day (Parent) */}
        {children.length > 0 && (
          <div className="bg-card rounded-2xl p-5 border border-border mb-4">
            <SectionToggle label="تعديل يوم محدد" icon={CalendarDays} show={showEditDay} onToggle={() => setShowEditDay(!showEditDay)} />
            {showEditDay && (
              <div className="space-y-3">
        ...
        {children.length > 0 && (
          <div className="mb-4">
            <SectionToggle label="المخطط الشهري" icon={CalendarDays} show={showChart} onToggle={() => setShowChart(!showChart)} />
        ...
        {children.length > 0 && (
          <div className="mb-4">
            <SectionToggle label="التقارير" icon={BarChart3} show={showReports} onToggle={() => setShowReports(!showReports)} />
            {showReports && <ReportView children={children} />}
          </div>
        )}
      </div>
